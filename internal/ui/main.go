...
func (e *editor) fileOpen() {
	open := dialog.NewFileOpen(func(read fyne.URIReadCloser, err error) {
		...
		e.LoadFile(read, "political")
	}, e.win)

	open.SetFilter(storage.NewExtensionFileFilter([]string{".json", ".civ5map"}))
	open.Show()
}
...
func (e *editor) buildMainMenu() *fyne.MainMenu {
	...
	file := fyne.NewMenu("File",
		...
		fyne.NewMenuItem("Export JSON...", e.exportJson),
	)
